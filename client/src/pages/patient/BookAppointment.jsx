...
const BookAppointment = ({ isModal = false, onClose }) => {
        ...
        const fetchData = async () => {
            try {
                ...
                const [servicesData, settingsData, exceptionsData] = await Promise.all([
                    ...
                    getPublicScheduleExceptions().catch(() => ({ data: [] }))
                ]);
    ...
    const isPastDate = (dateStr) => {
        ...
        // Parse YYYY-MM-DD manually to ensure local time (new Date("YYYY-MM-DD") treats as UTC)
    ...
    const dates = Array.from({ length: 30 }, (_, i) => {
        ...
        // Use local time components to avoid UTC shifts from toISOString()
    ...
    const handleConfirm = async () => {
        ...
        try {
            // Construct payload for Shift System
            const payload = {
                patient: {
                ...
                // Send shift preference
                shift: selections.shift.id, // 'morning' or 'evening'
                // appointment_at is used for date reference. 
                // We set it to the selected date with a dummy time to satisfy database/schema types if needed,
                // but strictly the date part matters.
                appointment_at: selections.date.iso,
                notes: patientDetails.notes.trim() || null
            };

            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/public/appointments`, {
                method: 'POST',
            ...
            <div style={styles.contentWrapper} className="content-wrapper">
                <div style={{ ...styles.progressContainer, padding: undefined }} className="progress-container">
                    <div className="flex items-center justify-between" style={{ marginBottom: '1.5rem' }}>
                        <Button variant="ghost" onClick={handleBack} disabled={step === 1 && !isModal} style={{ paddingLeft: 0, opacity: (step === 1 && !isModal) ? 0 : 1, gap: '0.25rem' }}>
                ...
                {step === 2 && (
                    <div className="animate-slide-up">
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: '0.75rem' }}>
                            {dates.slice(0, 15).map((d, i) => {
                                const isPast = isPastDate(d.iso);
                                const isSelected = selections.date?.iso === d.iso;
                                return (
                                    <Card
                                        key={i}
                                        hoverEffect={false} // Disable default, use custom class
                                        style={{
                                            ...styles.optionCard(isSelected),
                                    ...
                                    >
                                        <div style={{ color: isPast ? '#9ca3af' : 'var(--color-text-light)', marginBottom: '0.25rem', fontWeight: '600', fontSize: '0.85rem', textTransform: 'uppercase' }}>{d.day}</div>
                                        <div style={{ fontSize: '2rem', fontWeight: '800', color: isPast ? '#9ca3af' : 'var(--color-primary)', marginBottom: '0.25rem', lineHeight: 1 }}>{d.date}</div>
                                        <div style={{ fontSize: '0.8rem', color: isPast ? '#9ca3af' : 'var(--color-text)' }}>{d.month} {d.year}</div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.5rem', maxWidth: '800px', margin: '0 auto' }}>
                                ...
                                return (
                                    ...
                                    >
                                        <div style={{ textAlign: 'center', padding: '2rem' }}>
                                            <Sun size={64} style={{ marginBottom: '1.5rem', color: isMorningOpen ? '#f59e0b' : '#94a3b8' }} />
                                            <h3 style={{ fontSize: '1.5rem', fontWeight: '800', marginBottom: '0.5rem', color: isMorningOpen ? '#0f172a' : '#64748b' }}>Morning Shift</h3>
                                            <p style={{ fontSize: '1.1rem', color: isMorningOpen ? '#64748b' : '#94a3b8', marginBottom: '0.5rem' }}>
                                                {`${formatTime12(avail.morning_start)} - ${formatTime12(avail.morning_end)}`}
                                            </p>
                                            {!isMorningOpen && (
                                                <p style={{ fontSize: '0.9rem', color: '#ef4444', fontWeight: '600', fontStyle: 'italic' }}>
                                                    Shift unavailable on this date
                                                </p>
                                            )}
                                        </div>
                                    </Card>
                            ...
                            {(() => {
                                ...
                                return (
                                    ...
                                    >
                                        <div style={{ textAlign: 'center', padding: '2rem' }}>
                                            <Moon size={64} style={{ marginBottom: '1.5rem', color: isEveningOpen ? '#6366f1' : '#94a3b8' }} />
                                            <h3 style={{ fontSize: '1.5rem', fontWeight: '800', marginBottom: '0.5rem', color: isEveningOpen ? '#0f172a' : '#64748b' }}>Evening Shift</h3>
                                            <p style={{ fontSize: '1.1rem', color: isEveningOpen ? '#64748b' : '#94a3b8', marginBottom: '0.5rem' }}>
                                                {`${formatTime12(avail.evening_start)} - ${formatTime12(avail.evening_end)}`}
                                            </p>
                                            {!isEveningOpen && (
                                                <p style={{ fontSize: '0.9rem', color: '#ef4444', fontWeight: '600', fontStyle: 'italic' }}>
                                                    Shift unavailable on this date
